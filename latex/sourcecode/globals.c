...
/* true, wenn die Zeit gerade gemessen wurde und uebernommen werden kann */
volatile boolean got_time = false;
/* true, wenn gerade Minutenanfang gesucht wird */
volatile boolean search_time = false;

/* Vergleichswert fuer die Soft-PWM */
volatile byte cmp = 0;

/* Zum Taster entprellen, merken, ob er vorherige "Runde" schon gedrueckt war */
volatile byte buttonState[6];
/* Eine Tastersperre, dass nicht zu schnell hintereinander gedrueckt wird */
...
/* Um den richtigen Wochentag im Datumsstring anzeigen zu koennen */
char* weekdays[] = {
	"",
	"Montag",
	"Dienstag",
