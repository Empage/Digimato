...
static void initPorts() {
	 ...
	 */
	DDRA = 0;
	/* Pullup fuer die Taster aktivieren */
	PORTA |= 0b00111111;
	/*
	 * PB0: nc
	 * PB1: rote Debug LED
...
void initTimer2() {
	...
	/* CTC Wert: 147456 / 1024 / 100 = 144; -1 weil Intr erst 1 Timer Clock cycle spaeter ausgeloest wird */
	OCR2 = 144 - 1;
	/* Initialize counter */
	TCNT2 = 0;
}
...
ISR (TIMER2_COMP_vect) {
	...
	if (t2_purpose == ALARM) {
		...
		if (ret == T2_WAIT) {
			...
			/* wenn was schief gegangen ist, resette und fange neu an beim naechsten Interrupt */
...
void running_letters(char* str, byte time) {
	...
	for (int16_t i = 16; i >= (-6) * (int16_t)strlen(str); i--) {
		...
		/* Waehrend Beschreiben vom data-Array darf es nicht angezeigt werden */
		T0_DISABLE_INTR();
		clearAll();
...
static void horizontal_time(void) {
	byte tens = hour / 10;
	/* speichere den Wert der beiden Punkte, um zu blinken, weil clearAll alles loescht */
	byte p1 = data[2][8];
	byte p2 = data[4][8];
...
static inline void getButtonStates(void) {
	...
	for (byte button = BUT_BLACK_1; button <= BUT_BLUE_2; button++) {
		if (pressed(button)) {
			...
			/* Wenn er nur ein Zyklus gedrueckt wurde, deaktivere ihn wieder */
